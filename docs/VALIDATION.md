...
复验结果：短弧只跨相邻音、跨行关系两端各一小段、歌词与装饰不再相交。仍可见的
预期项：3 处 `2/1` 时值走 `muse.render.duration.unrepresentable` 只占宽不画延音线
（待 duration capability 裁决）；`||` 画普通单线并给诊断；一个 UnknownEvent 占位。
