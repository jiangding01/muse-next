...
- U06 作用域已裁决（2026-09-22，见 Appendix A U06 结案记录），两级证据：
  - **CONFIRMED**：body `L:` 只作用于它所在的声部，从该行起生效到该声部结束，不泄漏到其它声部。依据是只读探针对 5 个受影响语料文件（corpus#02/#03/#04/#10/#11）的复验——`[V:1]` 段内的 `L:1/4` 曾泄漏进 `[V:2]`，导致简谱声部每小节时值 = 拍号 × 2.000（100% 小节）；改成按声部作用域后 100% 回到 1.000×，TAB 声部与其余文件不受影响；与 Muse Pro 原版渲染结果一致。
  - **INFERRED**：同一声部被 `[V:n]` 交错成多段时，`L:` 按声部持续到该声部后续段（语料无跨段样本，"按声部持续" 与"按段重置"两种读法在全部语料上结果相同）。
  - 出现在任何声部上下文之前的 body `L:`（无归属）仍按 global 处理，对其后所有声部生效直到被覆盖；无 `V:` 的单声部文件（implicitSingle）行为不变。
...
2. **Domain Model 层**按字段类型区分：
   ...
   - **覆盖型**（`L:` `M:` `K:` `Q:`）：后者覆盖前者，但覆盖发生在**文档位置序**上，即 body 中的 `L:` 只影响其后的内容（§8.5）。
     - **`X:` 一并按覆盖型处理**（后者赢，发 info）。等级 `INFERRED`（依据：与其余非累加型字段一致；语料无 `X:` 重复样本，`X:` 本身仅 1 次出现）。
     - **注意 `K:` 的覆盖只在描述头内可能发生**：`K:` 同时是描述头终结符（§6.2），故第二条 `K:` 必然落在 body 区，按 §8.13 第 5 条处理，**不覆盖**已确定的调号。
...
5. 第 4 条对 **`K:`** 的一个直接后果：由于 `K:` 终结描述头（§6.2），一份文档里第二条及以后的 `K:` **一定**是 body 区字段，因此进 `ignoredFields` 并发 warning，`Score.key` 保持第一条 `K:` 的值。ABC 2.1（3.1.14）允许在 body 中变更调号，但 JCX 语料 0 样本、help 与 faq 零提及，**在拿到语料证据前不实现**（`UNVERIFIED`，登记为 Appendix A U10 的一个子情形）。
...
`CONFIRMED`（依据：ABC 2.1 §4.3 明确 `A3/` = `A3/2`；语料 pitch 声部（非 `style=tab`）1 次 / 1 文件：`corpus#05` 的 `D3/`）。此前 lexer 的 `DURATION_RE` 缺这一分支，导致 `D3/` 被误切成 `duration:"3"` + `duration:"/"` 两个游离叶子；已在 M1.5 T4 AST 预演中发现并修复。

**统计口径说明**：全语料对正则 `\d+/(?![\d/])` 的原始命中有 280 次、6 个文件，但绝大多数集中在 `corpus#02` / `corpus#04` 等文件的 `style=tab` 声部（如 `e0/d1/c2/...`），那是 `stringLetter + fret + tabDurSep` 的 TAB 记谱，与本节的 pitch 时值无关，不计入。剔除 TAB 声部后，仅 pitch 声部的真实命中为 1 次（上述 `corpus#05` 一例）。
...
`corpus#10` V:2（pitch/jianpu 声部）第 101 行含唯一一处（形态为 `...z2 \|\|\|2D2 ...`，前后旋律不引用） `\|<digit>` 字面形态（`\|\|\|2` 中的后两个 `\|` + `2`）。逐项核实：
...
- **和弦内的 `-`**：`CONFIRMED BY CORPUS`（M1.6 T7 预演，1173 枚 `-` 中 959 枚为和弦内形态）。形态为 `[X-Y-Z-]`——`-` 跟在**某个和弦成员**之后（可出现在和弦中间，不只是末尾，例：`[C-E-][EC]`、`[CD-]D`）。语义与 ABC 2.1 §4.11 一致：该 `-` 属于它前面那个成员，把这个成员连到**和弦之后**下一个事件里的**同音高**音，而**不是**连到同一和弦内的下一个成员，也**不是**按成员下标配对。目标只能是「跳过 barline 后的第一个 pitch 族事件」：是单音则须同音高，是和弦则须含同音高成员，否则记为未解析（`jcx.parse.tie.unresolved`）；组源逐成员结算，匹配不上的成员各记一条 unresolved。
  - 依据：按「组内相邻成员相连 + 按下标配对」实现时，11 文件语料出现 767 次两端音高不符、90 次成员数不匹配；改为本节裁决后两者均降为 0，未解析仅余 12 处（6 处为声部最后一个和弦之后无事件，6 处为 `=A` / `A` 这类临时记号拼写差异，见 §17）。
  - 临时记号：§17 的「小节内临时记号延续」语义未进 Domain，因此 `=A` 与其后的 `A` 目前不视为同音高，`UNVERIFIED`，留待实现 §17 作用域后复议。
...
**休止符不占用歌词音节位**（`INFERRED`）：help 未明说休止符是否算「音符」，语料 94 行
歌词未见休止符被赋词或占位的反例；实现按「休止符不是音符」裁决，把它排除在可对齐的
事件之外，但这是实现裁决而非 spec 明文，遇到时应发 diagnostic 而不是静默处理。
...
- **语料主形态是「组内 source → 组外 target」**：`CONFIRMED BY CORPUS ONLY`（24 枚中 22 枚，例：`{d8-S-}d10*6`、`{b8-H-b9-P-}b8`）——标记位于倚音 / 弦组的**末尾**，前一端是组的末成员，后一端是组**外**下一个 tabNote / tabGroup。只按「组内相邻成员」配对会丢掉这 22 枚。help 未描述该形态，故等级止于 corpus-only。
...
| ~~U06~~ | **已结案**（2026-09-22）：body 中 `L:` 的精确作用域裁定为「只作用于它所在的声部，不泄漏到其它声部」；行首 1 个空格是否有语义仍未验证，保留 UNVERIFIED | §8.5, §5.8 | 证据：语料内部小节时值自洽性（5 文件 corpus#02/#03/#04/#10/#11 100%）+ Muse Pro 原版渲染对照，2026-09-22；详见 §8.5 |
| U07 | 原版的默认速度（无 `Q:` 时） | §8.6 | 原版播放行为观测或反汇编常量 |
| U08 | `K:` 的模式 / 谱号扩展语法是否真被实现 | §8.7 | 含 `K:A Mix` / `K:A bass` 的原版样本 |
| U09 | 多条 `I:` 是否叠加显示 | §8.8 | 含 2 条以上 `I:` 的样本 |
| U10 | `M:` `K:` `Q:` `T:` `C:` 出现在 body 中的行为（含「第二条 `K:` 必然落在 body 区」这一子情形，§8.13 第 5 条：当前实现只忽略并发 warning，不做 ABC 式的 body 内调号变更） | §8.13, §8.12 | 相应样本 |
...
| U26 | `[<digit>` 跳房子与 `\|<digit>` / `\|0` 引号替代的实际语法 | §19.2 | `\|<digit>` 已更正：语料 11 个文件中 1 处疑似写法（`corpus#10` 第 101 行 `\|\|\|2`），但该声部全程零 `:\|`/`\|:` 反复标记、且与更早处一个不带数字的同旋律 `\|\|` 段落几乎逐字符重复，更可能是笔误而非跳房子记号（详见 §19.2 证据表），不构成有效实现证据，`不实现`。仍需含真实反复结构（`:\|`/`\|:` 与 `\|1`/`:\|2` 共现）的原版样本才能升级 |
