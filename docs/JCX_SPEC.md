...
help 与 ABC 2.1（4.3）在此完全一致：`A2` = 2 倍，`A/2` = 1/2，`A/` 简写 `A/2`，`A//` 简写 `A/4`，`A3/2` = 1.5 倍。

语料印证：`A,3/2`、`c/2`、`d/`、`G8`、`z/2`、`fx/`、`ax//`（TAB 中的 `//`）。
...
- `|<digit>` 与 `|0` 引号替代：`UNVERIFIED`，**不实现**，按 unknown token 保留。ABC 2.1 明确 `| 1`（带空格）不合法而 `| [1` 合法，这类细则在 JCX 中无任何证据支撑。
...
| `tab.jcx` | TAB 弦品 + `x` + `*N` 时值 | §26.2, §26.3, §26.5 |
...
| U26 | `[<digit>` 跳房子与 `\|<digit>` / `\|0` 引号替代的实际语法 | §19.2 | 含分房反复的原版样本（语料 11/11 零覆盖） |
...
| U36 | TAB 休止符的写法 | §26.8 | 含 TAB 休止的样本 |
