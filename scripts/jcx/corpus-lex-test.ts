 ...
 * 都是观测指标，不影响退出码。
 *
 * **round-trip 级**（第四级，M1.7 T7 + M1.8 T0/T1，逻辑在
 * `lib/roundtripInvariants.ts`）：对每个文件计算 preserve byte-identical /
 * line-identical、canonical 语义 round-trip（L2 投影相等）、canonical
 * 文本重解析后 diagnostics 无 error 级（`reparseClean`）、以及 canonical
 * document closure（`closure`：canonical 字节经 `loadJcx` → preserve 另存后
 * 逐字节回到原字节，且 canonical 不动点成立），外加 canonical 幂等
 * 与 reparse warning 计数观测。**失败条件四项**：byte-identical < 100%、
